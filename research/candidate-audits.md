...
- Canonical entry 是 `https://gitcode.com/Ascend/MindSpeed-MM`，read-only symref 确认 default branch 为 `master`；审计源码固定为 Ascend 官方 GitHub mirror 的 `0edd553e0ac9c912fe422c42cc9f42db9255ddcf`，提交时间 `2026-07-16T06:53:03Z`。固定树近期活跃，但该提交是 Qwen3.5 prefetch 修复，不是 Qwen3-TTS 训练变更。[已证实；SRC-018, SRC-019, SRC-048, SRC-100]
...
- 2026-07-17 canonical tag inventory 已解析：annotated `v1.0.0`、`v1.0.RC3.0`、`v2.0.0`、`v2.1.0`、`v2.2.0` 分别保留 tag-object→peeled-commit 双 SHA；lightweight `v2.3.0=d1e9f407...`、`v26.0.0=6c45b486...`。完整 ref/peeled 值在 ledger，moving tag refs 只作为检索时证据，不替代 fixed audit SHA。[已证实；SRC-100]
...
Revision control metadata 另确认 canonical default `master` 和七个公开 tag refs；这些 live refs 用于 release inventory，环境复现仍绑定 fixed `0edd553...`，不能由 tag 存在推导 CANN 8.5.2 compatibility。[已证实；SRC-100]
...
default branch、tag/ref inventory 与 fixed-tree 无 gitlink 已解析；缺失项是 exact hardware SKU/count/topology、driver/CANN/torch-npu/MindSpeed SHA、完整 command/log/tolerance、checkpoint checksum、converted-model inference、audio quality、multi-node artifact。没有本研究运行材料。[已证实/待真机验证；SRC-066, SRC-100]
...
主要固定证据：identity/tree/ref/tag `SRC-018, SRC-019, SRC-048, SRC-065, SRC-066, SRC-100`；guide/launch/config/train `SRC-020, SRC-021, SRC-022, SRC-023, SRC-043`；data/device/FSDP/op/checkpoint `SRC-067, SRC-068, SRC-069, SRC-070, SRC-071, SRC-072, SRC-073, SRC-074`；license `SRC-024, SRC-071`。
...
- Canonical entry 是 `https://gitcode.com/Ascend/MindSpeed-LLM`，read-only symref 确认 default branch 为 `master`；审计源码固定为官方 GitHub mirror `434baff794bd5594ebc9ed8a5b399110da9a44f0`，提交时间 `2026-07-16T09:26:04Z`，是 docs-only 变更。[已证实；SRC-025, SRC-026, SRC-049, SRC-101]
- 2026-07-17 较早检索时 canonical `master` 为 `2a545dd5dafba21d97b951af818b1646f538d247`，同日后续 symref 检索已前移为 `52caf88c08a0928011a2409b6c821a13617b99b5`；两者都与 fixed mirror `434baff...` 不同。即 **canonical ref 在审计日内移动且 canonical/mirror refs 已分叉**，不得覆盖任一时点或声称 fixed snapshot 等于当前 canonical/formal branch HEAD。[已证实；SRC-026, SRC-075, SRC-101]
...
- 2026-07-17 tag inventory 已解析：annotated `v1.0.0`、`v1.0.RC1.0`、`v1.0.RC2.0`、`v1.0.RC3.0`、`v2.0.0`、`v2.1.0` 保留 tag-object→peeled-commit 双 SHA；lightweight `v2.2.0=53f3639...`、`v2.3.0=1bdd970...`、`v26.0.0=57c3329...`。完整值在 ledger；live tags 不替代 fixed audit SHA。[已证实；SRC-101]
...
Revision control metadata 已确认 canonical default `master`、九个 tag refs 与同日两次不同 HEAD 观察；环境选择必须重新 pin formal ref 和外部依赖，不能把 moving `master` 当作复现锁。[已证实；SRC-075, SRC-087, SRC-101]

### 11. Reproduction evidence and missing evidence

default branch、tag/ref inventory 与 fixed-tree 无 gitlink 已解析；已有 fixed launch/config、training/checkpoint/data/conversion/inference/eval entry 与项目 ST/performance 材料。缺失本研究的 910B single-node 8-card、multi-node、resume、conversion、precision、performance 与稳定性运行；没有权重/数据下载。[已证实边界；SRC-026, SRC-027, SRC-028, SRC-044, SRC-045, SRC-046, SRC-076, SRC-101]
...
| Scale maturity /20 | **19** | SRC-027, SRC-079, SRC-081, SRC-083, SRC-084, SRC-085：TP/PP/CP/EP、distributed optimizer、FSDP2、8p/2-node configs 充分；扣 1 因缺少本研究独立的 scale/performance/stability/fault-recovery 运行，配置与项目材料不足以支撑满分。 |
...
| **Total** | **79/100** | preliminary 82 下调 3；角色保持 text-Qwen scale satellite。 |
...
主要固定证据：identity/ref/tag/tree `SRC-025, SRC-026, SRC-049, SRC-075, SRC-076, SRC-101`；launch/lifecycle `SRC-027, SRC-044, SRC-045, SRC-046, SRC-077, SRC-078, SRC-079, SRC-080, SRC-081, SRC-082, SRC-083, SRC-084, SRC-085`；environment/install `SRC-028, SRC-087`；license `SRC-029, SRC-086`。
...
Canonical/fixed source 为 `OpenMOSS/MOSS-TTS@ad99ec5f26debf1d6c1a4dc8461b2bcb787ec9af`；read-only symref 确认 default branch `main`，提交时间 `2026-06-22T11:51:50Z`，主题为 local v1.5 fine-tuning，维护近期且与训练相关。2026-07-17 canonical remote 未返回任何 `refs/tags/*`；这是已完成的 no-public-tags inventory，不是未解析状态。[已证实；SRC-030, SRC-050, SRC-102]

fixed recursive tree `truncated=false`，gitlink `moss_audio_tokenizer@56776e867cb38446fa4bc00d0aceccab5001b008`；其 fixed root LICENSE 为 Apache-2.0。parent 根 Apache-2.0 不 blanket-cover submodule，但 submodule 自身固定许可已识别为 Apache-2.0。[已证实；SRC-088, SRC-103, SRC-104]
...
`pyproject` 要求 Python `>=3.10`；项目材料推荐 Python 3.12、CUDA 12.8、PyTorch/Torchaudio 2.9.1、Transformers 5.0.0 与 Accelerate/DeepSpeed/FlashAttention。目标是 PyTorch 2.7.1 + CANN 8.5.2，device/runtime 有实质错位；CANN 8.5.2 行为 unknown。revision metadata 固定 default `main`、no-public-tags 结果与 tokenizer gitlink `56776e...`/Apache-2.0。[已证实/分析推断；SRC-089, SRC-090, SRC-091, SRC-102, SRC-103, SRC-104]

### 11. Reproduction evidence and missing evidence

default branch、no-public-tags inventory、fixed gitlink 与 submodule license 已解析。已有 fixed source、SFT guide、pins、single/DDP/FSDP/ZeRO-3 configs、launcher 与可加载 checkpoint layout；缺失 upstream Ascend/910B/torch-npu/HCCL、独立 run、raw logs/eval outputs 与完整 resume。未下载权重/数据。[已证实边界；SRC-030, SRC-031, SRC-088, SRC-089, SRC-090, SRC-091, SRC-092, SRC-093, SRC-102, SRC-103, SRC-104]
...
| Reproducibility /15 | **13** | SRC-030, SRC-031, SRC-050, SRC-088, SRC-089, SRC-090, SRC-091, SRC-092, SRC-093, SRC-102, SRC-103：fixed code/pins/data/checkpoint/ref/gitlink 强；扣 no resume 与 raw run/eval，不再以已解析 submodule SHA 扣分。 |
| Docs/license /10 | **10** | SRC-030, SRC-088, SRC-089, SRC-090, SRC-091, SRC-102, SRC-103, SRC-104：parent 与 fixed submodule 均 Apache-2.0 且文档强；no-public-tags 是已完成 inventory，不是许可歧义，submodule attribution 仍独立保留。 |
...
主要固定证据：tree/commit/ref/tag `SRC-030, SRC-050, SRC-102, SRC-103`；SFT/data `SRC-031, SRC-091, SRC-092, SRC-093`；dependency/submodule license `SRC-088, SRC-089, SRC-104`；model guide `SRC-090`。

| Claim state | Summary |
| --- | --- |
| 已证实 | speech SFT/data/checkpoint/config、default main、no-public-tags inventory、fixed gitlink、parent/submodule Apache-2.0 |
...
Canonical/fixed source 为 `FunAudioLLM/CosyVoice@074ca6dc9e80a2f424f1f74b48bdd7d3fea531cc`；read-only symref 确认 default branch `main`，提交时间 `2026-05-25T18:15:40Z`，主题为 ecosystem links。2026-07-17 tag inventory 解析 `flow_cache=fbab274b6a9abe127be61825092ca3a241bff19f` 与 `v2.0=8555549e882236e6541748b1042d95693caa82ba`；仓库活跃但 fixed HEAD 不是训练变更。[已证实；SRC-032, SRC-051, SRC-105]

fixed recursive tree `truncated=false`，gitlink `third_party/Matcha-TTS@dd9105b34bf2be2230f4aa1e4769fb586a3c824e`；其 fixed root LICENSE 为 MIT。parent 根 Apache-2.0 不 blanket-cover submodule，但 submodule fixed license 已识别，不再保留 SHA/许可 unknown；docs/license 的扣分只针对额外资产 provenance 边界。[已证实/分析边界；SRC-094, SRC-106, SRC-107]
...
fixed requirements 使用 CUDA 12.1 index，pins PyTorch/Torchaudio 2.3.1、Transformers 4.51.3、DeepSpeed 0.15.1、ONNX Runtime GPU 与 TensorRT。它与 PyTorch 2.7.1/CANN 8.5.2 有实质 device/version 差异，所有 target compatibility 均 unknown。revision metadata 固定 default `main`、两个 tags 与 Matcha-TTS gitlink `dd9105...`/MIT。[已证实/待真机验证；SRC-095, SRC-105, SRC-106, SRC-107]

### 11. Reproduction evidence and missing evidence

default branch、tag inventory、fixed gitlink 与 submodule license 已解析。已有 fixed source、LibriTTS staged recipe、CUDA pins、DDP/DeepSpeed args、dynamic batching、checkpoint average/export 与公开质量表；缺失 upstream Ascend/910B/torch-npu/HCCL、8-GPU/multi-node logs、完整 DDP optimizer resume、raw eval outputs 与本研究独立运行。未下载权重/数据。[已证实边界；SRC-032, SRC-033, SRC-094, SRC-095, SRC-096, SRC-097, SRC-098, SRC-099, SRC-105, SRC-106, SRC-107]
...
| Reproducibility /15 | **13** | SRC-032, SRC-033, SRC-051, SRC-095, SRC-096, SRC-097, SRC-098, SRC-099, SRC-105, SRC-106：fixed staged recipe/pins/checkpoint/export/ref/gitlink；扣 partial resume、default 1 GPU、raw logs 与 recipe 张力。 |
| Docs/license /10 | **9** | SRC-032, SRC-094, SRC-095, SRC-096, SRC-106, SRC-107：parent Apache-2.0 与 fixed Matcha-TTS MIT 已解析且文档强；扣 1 仅因其他预训练/导出资产 provenance 仍需逐项核查。 |
...
主要固定证据：tree/commit/ref/tag `SRC-032, SRC-051, SRC-105, SRC-106`；train/data/model `SRC-033, SRC-096, SRC-097, SRC-098, SRC-099`；dependency/submodule license `SRC-094, SRC-095, SRC-107`。

| Claim state | Summary |
| --- | --- |
| 已证实 | staged data/train/checkpoint/export、default main、two-tag inventory、fixed gitlink、parent Apache-2.0/submodule MIT |
...
审计总分排序是 CAND-001 **81**、CAND-002 **79**、CAND-003 **59**、CAND-004 **49**。CAND-001 与 CAND-002 只差 2 分，却代表实质不同路线：前者是 exact Qwen3-TTS Ascend/FSDP SFT，后者是 text-Qwen3 MindSpeed/MCore scale engineering。按 methodology，这不是可由总分静默解决的 tie，而是 **Task 6 major decision**；Task 5 不推荐最终主/卫星组合。[SRC-019, SRC-020, SRC-026, SRC-027]
